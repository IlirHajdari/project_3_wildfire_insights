...
d3.json("USGS2014.json").then((data) => {
    ...
    let states = [...new Set(data.map(d => d.State))].sort();
    ...
    function updateMap() {
        ...
        let heatArray = data
            .filter(incident => (selectedYear === "all" || incident.Year == selectedYear) &&
                                (selectedState === "all" || incident.State == selectedState))
            .map(incident => [incident.Latitude, incident.Longitude, incident["Fire Size in Acres"]]);
    ...
    legend.onAdd = function () {
        ...
        let labels = ["< 100", "100 - 1000", "1000 - 5000", "5000 - 10000", "> 10000"];
